...
export default function ProductsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (sorter.field === "price") {
      arr = [...arr].sort((a, b) =>
        sorter.order === "ascend" ? a.price - b.price : b.price - a.price
      );
    }
    return arr;
  }, [data, search, sorter]);
  ...
  const columns: TableProps<Product>["columns"] = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          <Button onClick={() => onEdit(record)}>Düzenle</Button>
          <Popconfirm
            title="Silinsin mi?"
            okText="Evet"
            cancelText="Vazgeç"
  ...
  const onTableChange: TableProps<Product>["onChange"] = (pagination, _filters, sort) => {
    ...
    if (!Array.isArray(sort)) {
      setSorter({
        field: (sort.field as string) ?? undefined,
  ...
  const onDelete = (key: string) => {
    setData(prev => prev.filter(x => x.key !== key));
    ...
    <Space direction="vertical" size="large" style={{ width: "100%" }}>
      ...
      >
        <Form form={form} layout="vertical" preserve={false}>
          <Form.Item name="name" label="Ürün Adı" rules={[{ required: true, message: "Gerekli" }]}>
            <Input />
          </Form.Item>
